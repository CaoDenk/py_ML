...
class RegressionHead(nn.Module):
    ...
    def _upsample(self, x: NestedTensor, disp_pred: Tensor, occ_pred: Tensor, scale: float):
        ...
        # upsample
        disp_pred = F.interpolate(disp_pred_attn[:, None, ], size=(h, w), mode='bilinear')  # N x 1 x H x W
        occ_pred = F.interpolate(occ_pred[:, None, ], size=(h, w), mode='bilinear')  # N x 1 x H x W
    ...
    def forward(self, attn_weight: Tensor, x: NestedTensor):
        ...
        if x.occ_mask is not None:
            ...
            occ_mask = x.occ_mask.squeeze_(0)[:, ::4, ::4]
            occ_mask_right = x.occ_mask_right.squeeze_(0)[:, ::4, ::4]
            # occ_mask = x.occ_mask
